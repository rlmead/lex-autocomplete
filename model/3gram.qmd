...
dat %>%
  ...
  mutate(qualitative_response = str_replace_all(qualitative_response, "\\s+", " ")) %>%
  filter(nchar(str_replace_all(qualitative_response, "\\s+", "")) >0 )-> corpus
```
...
comment_parse %>%
  filter(entity != "", 
         str_detect(entity, "_I"),
         str_detect(entity, "EVENT|FAC|GPE|LANGUAGE|LAW|LOC|NORP|ORG|PERSON|PRODUCT|WORK_OF_ART")) %>%
  count(token) %>% View
```
...
comment_parse %>%
  mutate(token = case_when(entity != '' & str_detect(token, "\\b[Aa]ve\\b") ~ "Avenue",
                           entity != '' & str_detect(token, "\\b[Bb]dwy\\b") ~ "Broadway",
                           entity != '' & str_detect(token, "\\b[Bb]lvd\\b") ~ "Boulevard",
                           entity != '' & str_detect(token, "\\b[Cc]o\\b") ~ "County",
                           entity != '' & str_detect(token, "\\b[Cc]t\\b") ~ "Court",
                           entity != '' & str_detect(token, "\\b[Ee]\\b") ~ "East",
                           entity != '' & str_detect(token, "\\b[Kk][Yy]\\b") ~ "Kentucky",
                           entity != '' & str_detect(token, "\\b[Ll]ex\\b") ~ "Lexington",
                           entity != '' & str_detect(token, "\\b[Ll]n\\b") ~ "Lane",
                           entity != '' & str_detect(token, "\\b[Mm]t\\b") ~ "Mount",
                           entity != '' & token == "Pkwy" ~ "Parkway",
                           entity != '' & str_detect(token, "\\b[Rr]d\\b") ~ "Road",
                           entity != '' & str_detect(token, "\\b[Ss]t\\b") ~ "Street",
                           TRUE ~ token)) %>%
...
The following are not allowed in firebase keys
...
sanitize <- function(x){
  x %>%
    ...
    str_replace_all("\\[", "<lbracket>") %>%
    str_replace_all("\\]", "<rbracket>") -> out
  return(out)
}
```
...
#| label: pre_probs
model %>%
  mutate(word_i = sanitize(word_i),
         word_j = sanitize(word_j),
         word_k = sanitize(word_k)) %>%
